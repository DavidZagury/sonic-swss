...
mod tests {
    ...
    fn mixed_snapshot(id: u16) -> IPFixTemplatesMessage {
        let mut update = hardware_template(
            id,
            &[
                (325, 8, None),
                (1, 8, Some(0x0001_0000)),
                (0, 4, Some(0)),
                (2, 8, Some(0x0015_0001)),
                (0, 8, Some(0)),
                (3, 8, Some(0x001a_0009)),
            ],
        );
        update.key = "profile|MIXED".into();
        // Metadata ordering need not follow template ordering. Queue and IPG
        // names may coincide; labels, not names, identify the objects.
        update.object_ids = Some(vec![3, 1, 2]);
        update.object_names = Some(vec![
            "Ethernet0|0".into(),
            "Ethernet0".into(),
            "Ethernet0|0".into(),
        ]);
    ...
    #[test]
    fn mixed_owner_decodes_all_groups_and_replaces_the_complete_snapshot() {
        let mut actor = actor();
        actor.handle_template(mixed_snapshot(256)).unwrap();
        let mut record = 42u64.to_be_bytes().to_vec();
        record.extend_from_slice(&10u64.to_be_bytes());
        record.extend_from_slice(&[0xaa; 4]);
        record.extend_from_slice(&11u64.to_be_bytes());
        record.extend_from_slice(&[0xbb; 8]);
        record.extend_from_slice(&12u64.to_be_bytes());
        let input = hardware_data(256, &[&record]);
        ...
        for (index, (name, type_id, stat_id, counter)) in [
            ("Ethernet0", 1, 0, 10),
            ("Ethernet0|0", 21, 1, 11),
            ("Ethernet0|0", 26, 9, 12),
        ]
        ...
        .enumerate()
        {
            let stat = decoded.stats.get(index).unwrap();
            assert_eq!(stat.object_name.as_ref(), name);
            assert_eq!(
                (stat.type_id, stat.stat_id, stat.counter),
                (type_id, stat_id, counter)
            );
        }

        // Removing groups publishes a replacement generation under the same owner.
        let mut port_only = hardware_template(257, &[(1, 8, Some(0x0001_0000))]);
        port_only.key = "profile|MIXED".into();
        port_only.object_names = Some(vec!["Ethernet0".into()]);
        actor.handle_template(port_only).unwrap();
        assert_eq!(actor.sessions.len(), 1);
        assert_eq!(actor.handle_record(&input).unwrap().counter_count(), 3);
        let replacement = hardware_data(257, &[&20u64.to_be_bytes()]);
        assert_eq!(actor.handle_record(&replacement).unwrap().counter_count(), 1);
        assert!(actor.handle_record(&input).unwrap().is_empty());
        actor
            .handle_template(IPFixTemplatesMessage::delete("profile|MIXED".into()))
            .unwrap();
        assert!(actor.sessions.is_empty());
        assert!(actor.handle_record(&replacement).unwrap().is_empty());
    }

    #[test]
    fn mixed_template_requires_complete_metadata_and_one_owner() {
        let mut actor = actor();
        let mut partial = mixed_snapshot(256);
        partial.key = "profile|PORT".into();
        partial.object_ids = Some(vec![1]);
        partial.object_names = Some(vec!["Ethernet0".into()]);
        assert!(actor
            .handle_template(partial)
            .unwrap_err()
            .to_string()
            .contains("unmapped object ID 2"));
        assert!(actor.installed.is_empty());

        actor.handle_template(mixed_snapshot(256)).unwrap();
        let mut duplicate = mixed_snapshot(256);
        duplicate.key = "profile|QUEUE".into();
        assert!(actor
            .handle_template(duplicate)
            .unwrap_err()
            .to_string()
            .contains("template collision"));
        assert_eq!(actor.sessions.len(), 1);
        assert_eq!(actor.installed.len(), 1);
        assert!(actor.sessions.contains_key("profile|MIXED"));
    }
